...
type PathTimingSearchFunc a = (a -> Path -> IO (a,[Path])) -> a -> Path -> IO Path

hill_climbing_search :: Int -> (a -> Path -> IO (a,[Path])) -> a -> Path -> IO Path
hill_climbing_search depth search_func aux_data path =
  hc_help depth Map.empty search_func aux_data path
  where
    hc_help ::  Int -> Map.Map Path Float -> (a -> Path -> IO (a,[Path])) -> a -> Path -> IO Path
    hc_help 0 _ _  aux_data path = return path
    hc_help depth time_lib search_func aux_data path =
      do -- IO
        (new_aux,new_paths) <- search_func aux_data path -- :: IO (a,[Path])
        ...
        if path == new_best_path then return path else
          hc_help (depth-1) updated_time_lib search_func new_aux new_best_path

exhaustive_search :: Int -> (a -> Path -> IO (a,[Path])) -> a -> Path -> IO Path
exhaustive_search depth search_func aux_data path =
  do -- IO
    all_paths <- ex_help depth search_func aux_data path :: IO [Path]
  ...
  where
    ...
    ex_help depth search_func aux_data path =
      do --IO
        (new_aux,new_paths) <- search_func aux_data path -- :: IO (a,[Path])
        expanded_paths <- sequence (do -- List
          new_path <- new_paths :: [Path]
          expanded_path <- return (ex_help (depth-1) search_func new_aux new_path) :: [IO [Path]]
