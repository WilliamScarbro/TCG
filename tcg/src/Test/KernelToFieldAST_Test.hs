...
ktfa_spec = do
  describe "compile Kernels to FAST" $ do
    it "compile Gamma to Diagonal to FAST" $ do
      ...
      (io_gk >>= (\x -> compileLOToFieldAST 0 x (inputVars 4))) `shouldReturn` (4,["t0","t1","t2","t3"],[FieldAssignment "t0" (FieldOpExpr (FMultiply (Constant 1) (Variable "x[0]"))),FieldAssignment "t1" (FieldOpExpr (FMultiply (Constant 9) (Variable "x[1]"))),FieldAssignment "t2" (FieldOpExpr (FMultiply (Constant 13) (Variable "x[2]"))),FieldAssignment "t3" (FieldOpExpr (FMultiply (Constant 15) (Variable "x[3]")))])
